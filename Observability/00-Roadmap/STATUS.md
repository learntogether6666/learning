...
- **Azure lab VM:** `vm-dcgm-lab` / `rg-dcgm-lab` / eastus / `20.25.43.14` — SSH key at `~/.ssh/dcgm_lab_key`. **Deallocate when not in use:** `az vm deallocate -g rg-dcgm-lab -n vm-dcgm-lab`. Restart: `az vm start -g rg-dcgm-lab -n vm-dcgm-lab`.
- **Next:**
  1. **DCGM Lab 01 exercises** — run failure injection scenarios (thermal spike, XID 79, ECC, straggler, power cap) one at a time; wire Grafana dashboard 12239
  2. **T4 quota request** — file via Azure portal for `Standard_NCASv3_T4` in eastus to get real GPU for future labs
  3. **Kind + DCGM** — local Kubernetes DCGM exporter DaemonSet lab (K8s integration path)
  4. **vLLM inference lab** — KV-cache pressure / continuous batching / goodput collapse
  5. **Tool-decision arena** — vLLM vs TRT-LLM vs SGLang
